  ...
  return (
    <div className="min-h-screen space-y-6 bg-[#f7f2e8] px-5 py-6 pb-40">
      <div className="rounded-[28px] bg-[#111111] p-6 text-white shadow-[0_10px_30px_rgba(0,0,0,0.25)]">
        ...
        <h1 className="mt-1 text-3xl font-extrabold tracking-tight">Moje dojo</h1>
        <p className="mt-2 text-sm text-white/70">Vyber telocvičňu a pokračuj</p>
      </div>

      {loading && (
        <div className="grid gap-5">
          {[1, 2, 3].map((i) => <div key={i} className="h-32 animate-pulse rounded-3xl bg-white/60" />)}
        </div>
      )}

      {!loading && dojos.length === 0 && (
        <div className="rounded-3xl bg-white p-6 text-center shadow-sm">Nemáš priradené žiadne dojo.</div>
      )}

      {!loading && dojos.length > 0 && (
        <div className="grid gap-5">
          {dojos.map((dojo) => (
            <Link key={dojo.id} href={`/dojos/${dojo.id}`} className="rounded-[26px] bg-white p-6 shadow-[0_8px_20px_rgba(0,0,0,0.08)] ring-1 ring-black/5 active:scale-[0.98]">
              ...
              <p className="mt-1 text-sm text-black/60">{dojo.address || "Bez adresy"}</p>
            </Link>
          ))}
        </div>
      )}
